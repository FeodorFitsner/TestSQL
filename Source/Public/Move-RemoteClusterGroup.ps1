Function Move-RemoteClusterGroup {
    <#
    .SYNOPSIS
    ...
    Param (
        ...
        [string[]]$ClusterGroup = "Cluster Group",
    ...
    $ClusterGroups = ForEach ($CG in $ClusterGroup)
    {
        Invoke-Command -ComputerName $Cluster -ScriptBlock {
            Import-Module FailoverClusters
            Get-ClusterGroup $Using:CG
        }
    }

    ForEach ($CG in $ClusterGroups)
    {
        #$RemoteNode = (Ping $($CG.OwnerNode) -n 1 | Select-String -Pattern "Pinging (.*) \[").Matches.Groups[1].Value
        $NodeMove = Invoke-Command -ComputerName $CG.PSComputerName -HideComputerName -ScriptBlock {
            Import-Module FailoverClusters
            If ($Using:CG.State -ne "Online")
            {
                Write-Verbose "$(Get-Date): Windows Cluster Failover: ""$($Using:CG.Name)"" is non-online state: $($Using:CG.State)"
